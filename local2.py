...
def kill_existing_session():
    subprocess.run(['tmux', 'kill-session', '-t', 'llama2'], stderr=subprocess.DEVNULL)

def create_and_setup_session():
    subprocess.run(['tmux', 'new-session', '-d', '-s', 'llama2'])
    # Set larger window size (adjust height and width as needed)
    # subprocess.run(['tmux', 'resize-window', '-t', 'llama2', '-x', '200', '-y', '1000'])
    
    commands = [
        'ssh root@172.232.34.231',
        'cd llama.cpp',
        './llama-cli -m /root/models/DarkIdol-Llama-3.1-8B-Instruct-1.2-Uncensored-Q8_0-imat.gguf -cnv -mli'
    ]
    
    for cmd in commands:
        subprocess.run(['tmux', 'send-keys', '-t', 'llama2', cmd, 'Enter'])

def get_tmux_output():
    try:
        return subprocess.check_output(['tmux', 'capture-pane', '-t', 'llama2', '-p']).decode()
    except:
        return ""
...
def main():
    ...
    #     user_input = input(">> ")
    #     subprocess.run(['tmux', 'send-keys', '-t', 'llama2', user_input, 'Enter'])
